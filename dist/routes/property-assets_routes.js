...
router.get("/images/:propertyId", async (req, res) => {
...
// POST /api/amenities
router.post("/amenities", authenticate, async (req, res) => {
    try {
        if (!req.body || Object.keys(req.body).length === 0) {
            return res.status(400).json({ error: "Request body is required" });
        }
...
router.delete("/amenities/:id", authenticate, async (req, res) => {
    const id = parseId(req.params.id);
    if (!id) {
...
router.get("/property-amenities/:propertyId", async (req, res) => {
    const propertyId = parseId(req.params.propertyId);
    if (!propertyId) {
        return res.status(400).json({ error: "Invalid property ID" });
    }
...
router.post("/property-amenities", authenticate, async (req, res) => {
    try {
        if (!req.body || Object.keys(req.body).length === 0) {
            return res.status(400).json({ error: "Request body is required" });
        }
...
router.delete("/property-amenities/:id", authenticate, async (req, res) => {
    const id = parseId(req.params.id);
    if (!id) {
...
//# sourceMappingURL=property-assets_routes.js.map