...
// ─────────────────────────────────────────────
router.get("/owner/:ownerId", authenticate, verifyOwnership("ownerId"), async (req, res) => {
...
router.post("/", authenticate, async (req, res) => {
    try {
        ...
        const newProperty = await db
            ...
            owner_id: req.user.id, // derived from auth, never from client
        })
            .returning();
...
// ─────────────────────────────────────────────
router.put("/:id", authenticate, verifyOwnership("id"), async (req, res) => {
    const id = parseId(req.params.id);
    if (!id) {
        return res.status(400).json({ error: "Invalid property ID" });
    }
    try {
        ...
        if (req.body.title !== undefined)
            allowedUpdate.title = req.body.title;
        if (req.body.description !== undefined)
            allowedUpdate.description = req.body.description;
        if (req.body.property_type !== undefined)
            allowedUpdate.property_type = req.body.property_type;
        if (req.body.address !== undefined)
            allowedUpdate.address = req.body.address;
        if (req.body.city !== undefined)
            allowedUpdate.city = req.body.city;
        if (req.body.state !== undefined)
            allowedUpdate.state = req.body.state;
        if (req.body.zip_code !== undefined)
            allowedUpdate.zip_code = req.body.zip_code;
        if (req.body.country !== undefined)
            allowedUpdate.country = req.body.country;
        if (req.body.latitude !== undefined)
            allowedUpdate.latitude = req.body.latitude;
        if (req.body.longitude !== undefined)
            allowedUpdate.longitude = req.body.longitude;
        if (req.body.bedrooms !== undefined)
            allowedUpdate.bedrooms = req.body.bedrooms;
        if (req.body.bathrooms !== undefined)
            allowedUpdate.bathrooms = req.body.bathrooms;
        if (req.body.area_sqft !== undefined)
            allowedUpdate.area_sqft = req.body.area_sqft;
        if (req.body.monthly_rent !== undefined)
            allowedUpdate.monthly_rent = req.body.monthly_rent;
        if (req.body.security_deposit !== undefined)
            allowedUpdate.security_deposit = req.body.security_deposit;
        if (req.body.is_furnished !== undefined)
            allowedUpdate.is_furnished = req.body.is_furnished;
        if (req.body.pets_allowed !== undefined)
            allowedUpdate.pets_allowed = req.body.pets_allowed;
        if (req.body.smoking_allowed !== undefined)
            allowedUpdate.smoking_allowed = req.body.smoking_allowed;
        if (req.body.parking_available !== undefined)
            allowedUpdate.parking_available = req.body.parking_available;
        if (req.body.lease_duration_months !== undefined)
            allowedUpdate.lease_duration_months = req.body.lease_duration_months;
...
// ─────────────────────────────────────────────
router.delete("/:id", authenticate, verifyOwnership("id"), async (req, res) => {
    const id = parseId(req.params.id);
    if (!id) {
        return res.status(400).json({ error: "Invalid property ID" });
    }
...
//# sourceMappingURL=properties_routes.js.map